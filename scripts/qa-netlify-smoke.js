...
try{
  ...
  await page.route(base+'/api/health',route=>route.fulfill({contentType:'application/json',body:JSON.stringify({...health,rentcastConfigured:false})}));await page.reload();assert.equal(await page.getByRole('button',{name:'Analyze Property',exact:true}).isDisabled(),true);await page.getByRole('button',{name:'Florida Portfolio / Fantasia',exact:true}).click();await page.locator('[data-testid="state"]').filter({hasText:'READY_PROPERTY'}).waitFor();checks.push('Missing-key health disables live action; fixtures still work');
