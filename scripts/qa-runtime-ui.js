...
try{
  ...
  await page.getByRole('button',{name:'Analyze Property',exact:true}).click();await page.waitForFunction(()=>document.querySelector('[data-testid="state"]')?.dataset.state==='READY_PROPERTY');
  ...
  await page.getByRole('button',{name:'Create Deal Analysis',exact:true}).click();await page.waitForFunction(()=>document.querySelector('[data-testid="state"]')?.dataset.state==='READY_DEAL');assert.equal(await page.locator('#economics').count(),0);
  await page.getByText('Enter a separate deal scenario',{exact:true}).click();
  for(const [key,value]of Object.entries({acquisition:160000,repairs:10000,sale:279000,holdDays:90,rent:3000,commission:5000,acquisitionClosingCosts:1000,dispositionClosingCosts:2000}))await page.locator(`.deal-form input[name="${key}"]`).fill(String(value));
  await page.getByRole('button',{name:'Analyze explicit claims'}).click();await page.locator('#economics').waitFor();assert.match(await page.locator('#economics').innerText(),/\$98,000/);assert.equal(providerCalls,2);checks.push('Create Deal starts blank; explicit analyst claims use explicit closing timing with no extra provider calls');
  await page.getByRole('button',{name:'Analyze Property',exact:true}).click();await page.waitForFunction(()=>document.querySelector('[data-testid="state"]')?.dataset.state==='READY_PROPERTY');assert.match(await page.locator('.cache').innerText(),/Cached evidence/);assert.equal(providerCalls,2);checks.push('Explicit repeated analysis uses cache');
  await page.route(`${app.url}/api/resolve`,route=>route.fulfill({status:404,contentType:'text/html',body:'<html>SPA fallback</html>'}));
  await page.getByRole('button',{name:'Analyze cached evidence'}).click();await page.getByRole('status').filter({hasText:'API is unavailable'}).waitFor();assert.doesNotMatch(await page.locator('[data-testid="state"]').innerText(),/Unexpected token/);checks.push('HTML fallback displays sanitized service-unavailable error and clears stale property');
